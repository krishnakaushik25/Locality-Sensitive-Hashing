...
def score_fn(data, count_path, features, list_cols, seed_ids,
    ...
    # Read the feature count flie
    count_df = pd.read_csv(count_path)
    # Select user records from see set
    seed_df = data[data["id"].isin(seed_ids)]
    # Calculate feature count in seed set
...
def get_extn(data, seed_ids, label, extn_path, x=2):
    ...
    """
    # Drop users who don't have the score (those are not neighbors)
    data = data.dropna(subset=["score"])
    # Sort the users by score
    data = data.sort_values(by="score", ascending=False)
    # Select top users
    extn = data.iloc[:x*len(seed_ids),:][["id"]]
    # Write the extended user IDs to disc
    extn.to_csv(extn_path, index=False)
    # Calculate the click rate of extended user set
